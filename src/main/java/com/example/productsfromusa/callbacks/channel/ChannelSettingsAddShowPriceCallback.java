...
@Component
public class ChannelSettingsAddShowPriceCallback implements CallbackHandler {
    ...
    public TelegramMessage apply(Update update) {
        ...
        String id = callbackData.substring(CallbackType.ADD_SHOW_PRICE.length());
        ...
        if (channel != null) {
            ...
            if (user.getState().equals(States.CHANNEL_SETTINGS)) {
                user.setState(States.SHOW_PRICE);

                userService.saveUser(user);

                String text = EmojiParser.parseToUnicode(":moneybag: Отображать цену?");
                ...
                channelButton.setCallbackData(CallbackType.ADD_SHOW_PRICE3 + channel.getId());
                rowInLine.add(channelButton);
