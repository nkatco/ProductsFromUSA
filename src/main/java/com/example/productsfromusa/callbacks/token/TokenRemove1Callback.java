...
@Component
public class TokenRemove1Callback implements CallbackHandler {
    ...
    public TelegramMessage apply(Update update) {
        ...
        String id = callbackData.substring(CallbackType.REMOVE_TOKEN1.length());
        ...
        if (token != null && token.isActive()) {
            ...
            if (user.getState().equals(States.CHANNEL_SETTINGS)) {
                ...
                message.setText("Ваш токен деактивируется, когда истечет его срок действия (" + dateTime.format(formatter) + ") он удалится полностью. Чтобы прекратить отправку анонсов по нему, удалите действующие записи анонсов.\n\nВы уверены, что хотите удалить токен? (это действие нельзя будет отменить)");
                ...
                tokenButton.setText("Уверен");
                tokenButton.setCallbackData(CallbackType.REMOVE_TOKEN2 + token.getId());
                ...
                tokenButton.setCallbackData(CallbackType.INFO_TOKEN + token.getId());
                rowInLine.add(tokenButton);
                rowsInLine.add(rowInLine);
                rowInLine = new ArrayList<>();
